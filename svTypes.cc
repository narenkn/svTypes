...
namespace svDpiExtn {
  ...
  svBitSliceT::svBitSliceT(svBitVec32 *b, uint16_t l, uint16_t r)
    : Value(b), msb(l), lsb(r) {
  ...
  svBitSliceT::getValue (svBitVec32 *V) const
  {
    ...
    for(uint32_t ui1=SV_CANONICAL_SIZE(lsb+1)-1; ui1<SV_CANONICAL_SIZE(msb+1); ui1++, v_idx++) {
      V[v_idx] = (Value[ui1] >> lsb_mod) & lsb_shift_mask;
      //naren		if (v_idx) {
      //naren			std::cout << " V[-1]:" << std::hex << V[v_idx-1] <<
      //naren				" Value[ui1]:" << ((Value[ui1]<<((sizeof(svBitVec32)<<3)-lsb_mod))&(((svBitVec32)-1)<<((sizeof(svBitVec32)<<3)-lsb_mod))) << std::endl ;
      //naren		}
      if ((v_idx) && lsb_mod) {
	V[v_idx-1] |= ((Value[ui1]<<((sizeof(svBitVec32)<<3)-lsb_mod))&(((svBitVec32)-1)<<((sizeof(svBitVec32)<<3)-lsb_mod)));
      }
      //naren		std::cout << "Debug 2:v_idx:" << v_idx << " ui1:" << ui1 <<
      //naren			" lsb_mod:" << lsb_mod << " V[]:" << std::hex << V[v_idx];
      //naren		if (v_idx)
      //naren			std::cout << " V[-1]:" << std::hex << V[v_idx-1];
      //naren		std::cout << " Value[]:" << std::hex << Value[ui1] << std::endl;
    }

    /* Last word has to be fixed */
    if ((1 == v_idx) && (SV_CANONICAL_SIZE(lsb+1) != SV_CANONICAL_SIZE(msb+1))) {
      //naren		std::cout << "  v_idx:" << v_idx << std::endl;
      V[v_idx-1] |= (Value[SV_CANONICAL_SIZE(msb+1)]<<((sizeof(svBitVec32)<<3)-lsb_mod)) & (((svBitVec32)-1) << ((sizeof(svBitVec32)<<3)-lsb_mod));
      assert(v_idx);
      //		V[v_idx-1] |= (Value[SV_CANONICAL_SIZE(msb+1)]<<((sizeof(svBitVec32)<<3)-lsb_mod));
    } //else {
...
#if 0
    ctypes_copy_bit_parray(V, Value, 64, msb-lsb+1, 0, lsb);
#else
    //	for (uint32_t ui1=0; ui1<B.uArraySize; ui1++)
    //		std::cout << "Value" << ui1 << ":" << Value[ui1] << std::endl;
    getValue(V);
#endif
  ...
  svBitSliceT::operator=(const std::string &rhs)
  {
    ...
    for (uint32_t ui1=lsb; msb>=ui1; ui1++) {
      Value[SV_CANONICAL_SIZE(ui1+1)-1] &= ~(((svBitVec32)1)<<(ui1%(sizeof(svBitVec32)<<3)));
    }
    ...
    setUsingBinaryString(Value, msb, lsb, result);
  ...
  {
    return svBitSliceT(this->Value, l_idx, r_idx);
  }
  ...
  svBitSliceT
  svBitP::operator()(uint32_t l_idx, uint32_t r_idx)
  {
    return svBitSliceT(this->Value, l_idx, r_idx);
  }
...
#ifdef SVLOGIC
  svLogicSliceT::svLogicSliceT(svLogicVec32* b, uint16_t l, uint16_t r)
    : Value(b), msb(l), lsb(r) {
    /* The following assertions should fail for out of uWordSize,
       wrong usage */
    assert (msb >= lsb);
  ...
  svLogicSliceT::getValue (svLogicVec32 *V) const
  {
    ...
    v_idx = 0;
    for(uint32_t ui1=SV_CANONICAL_SIZE(lsb+1)-1; ui1<SV_CANONICAL_SIZE(msb+1); ui1++, v_idx++) {
      V[v_idx].c = (Value[ui1].c >> lsb_mod) & lsb_shift_mask;
      V[v_idx].d = (Value[ui1].d >> lsb_mod) & lsb_shift_mask;
      //FIXME		if (v_idx) {
      //FIXME			std::cout << " V[-1]:" << std::hex << V[v_idx-1] <<
      //FIXME				" Value[ui1]:" << ((Value[ui1]<<((sizeof(svBitVec32)<<3)-lsb_mod))&(((svBitVec32)-1)<<((sizeof(svBitVec32)<<3)-lsb_mod))) << std::endl ;
      //FIXME		}
      if ((v_idx) && lsb_mod) {
	V[v_idx-1].c |= ((Value[ui1].c<<((sizeof(svBitVec32)<<3)-lsb_mod))&(((svBitVec32)-1)<<((sizeof(svBitVec32)<<3)-lsb_mod)));
	V[v_idx-1].d |= ((Value[ui1].d<<((sizeof(svBitVec32)<<3)-lsb_mod))&(((svBitVec32)-1)<<((sizeof(svBitVec32)<<3)-lsb_mod)));
      }
      //FIXME		std::cout << "Debug 2:v_idx:" << v_idx << " ui1:" << ui1 <<
      //FIXME			" lsb_mod:" << lsb_mod << " V[]:" << std::hex << V[v_idx];
      //FIXME		if (v_idx)
      //FIXME			std::cout << " V[-1]:" << std::hex << V[v_idx-1];
      //FIXME		std::cout << " Value[]:" << std::hex << Value[ui1] << std::endl;
    }

    /* Last word has to be fixed */
    if ((1 == v_idx) && (SV_CANONICAL_SIZE(lsb+1) != SV_CANONICAL_SIZE(msb+1))) {
      //FIXME		std::cout << "  v_idx:" << v_idx << std::endl;
      V[v_idx-1].c |= (Value[SV_CANONICAL_SIZE(msb+1)].c<<((sizeof(svBitVec32)<<3)-lsb_mod)) & (((svBitVec32)-1) << ((sizeof(svBitVec32)<<3)-lsb_mod));
      V[v_idx-1].d |= (Value[SV_CANONICAL_SIZE(msb+1)].d<<((sizeof(svBitVec32)<<3)-lsb_mod)) & (((svBitVec32)-1) << ((sizeof(svBitVec32)<<3)-lsb_mod));
      assert(v_idx);
    } //else {
    v_idx = (SV_CANONICAL_SIZE(msb-lsb+1));
    assert(msb>=lsb);
  ...
  svLogicSliceT::operator=(const std::string &rhs)
  {
    /* initialize myself to 0 */
    for (uint32_t ui1=lsb; msb>=ui1; ui1++) {
      Value[SV_CANONICAL_SIZE(ui1+1)-1].c &= ~(((svBitVec32)1)<<(ui1%(sizeof(svBitVec32)<<3)));
      Value[SV_CANONICAL_SIZE(ui1+1)-1].d &= ~(((svBitVec32)1)<<(ui1%(sizeof(svBitVec32)<<3)));
    }
    if (0 == rhs.length()) return *this;
  ...
  svLogicSliceT::operator=(const uint64_t &rhs)
  {
    ...
    for (uint16_t ui1=0; (ui1 < (sizeof(uint64_t)<<3)) && (l<=msb); ui1++, l++, l_rhs>>=1) {
      uint16_t l_mod = l % (sizeof(svBitVec32)<<3);
      Value[SV_CANONICAL_SIZE(l+1)-1].d &= ~(((svBitVec32)1)<<l_mod);
      Value[SV_CANONICAL_SIZE(l+1)-1].d |= (l_rhs&1)<<l_mod;
      Value[SV_CANONICAL_SIZE(l+1)-1].c &= ~(((svBitVec32)1)<<l_mod);
    }
  ...
  {
    return svLogicSliceT(this->Value, l_idx, r_idx);
  }
  ...
  setUsingBinaryString(svLogicVec32 *V, uint16_t msb, uint16_t lsb, const std::string& binStr )
  {
	...
	(svBitVec32)((binStr[strIndex]=='z') ?
		     sv_1:( (binStr[strIndex]=='x')?sv_1:(0)
			    )) << (lsb%(sizeof(svBitVec32)<<3));
      V[SV_CANONICAL_SIZE(lsb+1)-1].d |= 
	(svBitVec32)((binStr[strIndex]=='1') ?
		     sv_1:( (binStr[strIndex]=='x')?sv_1:(0)
			    )) << (lsb%(sizeof(svBitVec32)<<3));
    }
  }
